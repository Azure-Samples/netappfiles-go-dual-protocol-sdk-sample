			...
			Rules: []*armnetapp.ExportPolicyRule{
				{
					...
					Cifs:           to.Ptr(map[bool]bool{true: true, false: false}[utils.Contains(protocolTypes, cifs)]),
					Nfsv3:          to.Ptr(map[bool]bool{true: true, false: false}[utils.Contains(protocolTypes, nfsv3)]),
					Nfsv41:         to.Ptr(map[bool]bool{true: true, false: false}[utils.Contains(protocolTypes, nfsv41)]),
...
func UpdateANFVolume(ctx context.Context, location, resourceGroupName, accountName, poolName, volumeName string, volumePropertiesPatch armnetapp.VolumePatchProperties, tags map[string]*string) (*armnetapp.Volume, error) {
	...
	resp, err := future.PollUntilDone(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("cannot get the volume update future response: %v", err)
	}
...
func MoveANFVolumeToNewPool(ctx context.Context, resourceGroupName, accountName, poolName, volumeName string, newPool armnetapp.PoolChangeRequest) error {
	...
	future, err := volumeClient.BeginPoolChange(
		ctx,
		resourceGroupName,
		accountName,
		poolName,
